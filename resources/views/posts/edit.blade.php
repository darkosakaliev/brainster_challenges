...
<form method="POST" action="{{ route('posts.update', $post) }}">
    ...
    <div class="mb-6">
        ...
        <x-input id="url" class="block mt-1 w-full" type="url" name="url" :value="$post->url" required autofocus />
    </div>
    <div class="mb-6">
        <x-label for="description" :value="__('Description')" />
        <textarea id="description" class="rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 block mt-1 w-full" type="text" name="description" required autofocus>{{$post->description}}</textarea>
    </div>
    <div class="mb-6">
